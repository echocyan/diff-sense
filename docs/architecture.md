...
`file_read`、`code_search` 与锚定读取同一版本的代码：workspace 模式读工作区（搜索包含未跟踪文件，遵守 `.gitignore`），commit 模式读该提交，range 模式读 `to` 端的提交。因此审查历史提交或在 CI 中审查 PR 时，Agent 看到的就是被审查的代码，而不是当前检出的版本。
