...
- `diff-sense config` — interactive wizard when no args (built with @clack/prompts: provider → model → API key; an empty API key keeps the saved one only when the provider is unchanged; all answers are written in one atomic write); `config set <key> <value>` / `config get <key>` for non-interactive (`get` exits 1 when the key is unset)
...
- **GitHub**: `{ review, summary, fallback }` for the GitHub Action. `review` is the Pull Request Review API request body (`event: "COMMENT"`, one comment per finding whose whole range lies inside a single hunk's new side), or `null` when there are none; `summary` is the Markdown for a PR conversation comment listing every other finding (unanchored, outside the hunks, or spanning hunks), or `null`. The Review API rejects the whole request if any comment falls outside the diff, so this split is decided against the reviewed diff in the deterministic layer. `fallback` is the Markdown for a PR conversation comment listing the inline findings, posted instead when the Review API still rejects the request (`null` when `review` is `null`); all comment Markdown is generated by the CLI.
